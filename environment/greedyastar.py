    ...
    def init_distance(self):
        for i in range(config.Source_num):
            for j in range(config.Hole_num):
                ...
                self.dis_sum[i][j] = np.array([dis,dis])
                self.dis_average[i][j] = self.dis_sum[i][j]
        ...
        for i in range(self.agent_num):
            if self.end_update[i]:
                ...
                if to_hole:
                    # print "find source", i
                    for j in range(config.Source_num):
                        if self.dis_average[j][self.start_pos[i] - config.Source_num][0] < min_dis:
                            min_dis = self.dis_average[j][self.start_pos[i]- config.Source_num][0]
                            min_index = j
                    self.end_pos[i] = min_index
