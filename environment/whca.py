...
class WHCA:
    ...
    def getScheduleTable(self, total_time_step):
        table = [[[0 for t in range(total_time_step)] for y in range(config.Map.Height)] for x in range(config.Map.Width)]
    ...
    def getScheduleTable4(self, step):
        table = np.zeros((config.Map.Width, config.Map.Height,8))
        for entry in self.reserve:
            [pos, t] = decode(entry)
            [x, y] = pos
            t-=(step-4)
