...
class FileLogger(Callback):
    def __init__(self, filepath, interval=None):
        ...
        # Some algorithms compute multiple
        # episodes at once since they are multi-threaded.
    ...
    def on_episode_end(self, episode, logs):
        ...
        data += [('episode', episode + 1), ('duration', duration),
                 ('episode_wins', self.env.episode_wins)]
    ...
    def save_data(self):
        ...
        for key, values in self.data.items():
            ...
            # We convert to np.array() and then
            # to list to convert from np datatypes to native datatypes.
            # This is necessary because json.dump cannot handle np.float32, for
            # example.
            sorted_data[key] = np.array(
                [self.data[key][idx] for idx in sorted_indexes]).tolist()

        # Overwrite already open file. We can
        # simply seek to the beginning since the file will
